...
struct [[eosio::table, eosio::contract("system")]] user_resources {
   name          owner;
   asset         net_weight;
   asset         cpu_weight;
   int64_t       ram_bytes = 0;
 ...
 */
struct [[eosio::table, eosio::contract("system")]] delegated_bandwidth {
   name          from;
   name          to;
   asset         net_weight;
   asset         cpu_weight;
...
struct [[eosio::table, eosio::contract("system")]] refund_request {
   name            owner;
 ...
 */
void system::buyrambytes( name payer, name receiver, uint32_t bytes ) {
 ...
 */
void system::buyram( name payer, name receiver, asset quant ) {
   ...
   if( fee.amount > 0 ) {
      _token.transfer(payer, ramfee_account, extended_asset(fee, _self), "ram fee");
   }

   _token.authorization = {{ram_account, active_permission}, {payer, active_permission}};
   _token.transfer(payer, ram_account, extended_asset(quant_after_fee, _self), "buy ram");
 ...
 *  for RAM over time.
 */
void system::sellram( name account, int64_t bytes ) {
   check(false, "not activated action");
   ...
   if( fee > 0 ) {
      _token.transfer(account, ramfee_account, extended_asset(asset(fee, core_symbol()), _self), "sell ram fee");
   }

   _token.authorization = {{ram_account, active_permission}, {account, active_permission}};
   _token.transfer(ram_account, account, extended_asset(tokens_out, _self), "sell ram");
}

void system::changebw( name from, name receiver,
   ...
   if ( stake_account != source_stake_from ) { //for eosio both transfer and refund make no sense
      ...
      if ( 0 < transfer_amount.amount ) {
         token().transfer(source_stake_from, stake_account, extended_asset(transfer_amount, default_account), "stake bandwidth");
      }
   }
}

void system::delegatebw( name from, name receiver,
...
void system::undelegatebw( name from, name receiver,
...
void system::refund( const name owner ) {
   ...
   token _token;
   _token.authorization = {{stake_account, active_permission}, {req->owner, active_permission}};
   _token.transfer(stake_account, req->owner, extended_asset(req->net_amount + req->cpu_amount, default_account), "unstake");

   refunds_tbl.erase( req );
}

void system::setramgift(uint8_t kbytes) {
   require_auth(_self);
